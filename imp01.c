...
/*                                                                                  */
/*  Reseni vytvoril a odevzdava: (Jan Osusky, xosusk00)*/
/*                                                                                  */
/************************************************************************************/
...
unsigned char corrl[9] = "xlogin00"; // Vas login
unsigned char login[9];              // misto pro prijaty retezec

/* Inicializace MCU - zakladni nastaveni hodin, vypnuti watchdogu */
void MCUInit(void)
{
    MCG->C4 |= (MCG_C4_DMX32_MASK | MCG_C4_DRST_DRS(0x01)); // nastaveni frekvence na 48 mgh zapinani hodin
    SIM->CLKDIV1 |= SIM_CLKDIV1_OUTDIV1(0x00);              // delicka hodinoveho signalu 2 na 0
    SIM->COPC = SIM_COPC_COPT(0x00);                        // watchdog vypnut -> 0 // strana 65
}

/* Inicializace pinu pro vysilani a prijem pres UART - RX a TX */
void PinInit(void)
{
    SIM->SOPT2 |= SIM_SOPT2_UART0SRC(0x01); // nastaveni zdroje hodin, zapnuti prefiriim pomoci zapnuti hodin
    SIM->SCGC5 |= SIM_SCGC5_PORTB_MASK;     // Zapnout hodiny pro PORTA a PORTB // str 183
    SIM->SCGC4 |= SIM_SCGC4_UART0_MASK;     // povoleni hodin

    PORTB->PCR[1] = (0 | PORT_PCR_MUX(0x02)); // UART0_TX // str 148 konfigurace vyvodu portu
    PORTB->PCR[2] = (0 | PORT_PCR_MUX(0x02)); // UART0_RX

    PORTB->PCR[13] = (0 | PORT_PCR_MUX(0x01)); // Beeper (PTB13) nastaveni portu na
    PTB->PDDR = GPIO_PDDR_PDD(0x2000);         // "1" znamena, ze pin bude vystupni musi se natavit
}

/* Inicializace UART - nastaveni prenosove rychlosti 115200Bd, 8 bitu, bez parity */
void UART0Init(void)
{
    UART0->C2 &= ~(UART0_C2_TE_MASK | UART0_C2_RE_MASK); // deaktivace vysilace a primace // znena parametru

    UART0->BDH = 0x00;
    UART0->BDL = 0x1A; // Baud rate 115 200 Bd, 1 stop bit
    UART0->C4 = 0x0F;  // Oversampling ratio 16, match address mode disabled // vzorkovaci frekvence 16* vyssi nez frekvence konumikace
